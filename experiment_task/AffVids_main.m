...
%% Timing variables
mvfac = 10;%mouse sensitivity
response_duration = 4;%how long do people get to respond to the questions
%stimulus_duration = 5; % shorten the video when testing this script!!!
stimulus_duration = 20; %how long do videos last
% trial_duration = 36;

pre_stim_jitter_social = [1,1,2,2];%time for after trial jitter
pre_stim_jitter_spiders = [1,1,2,2];
pre_stim_jitter_heights = [1,1,2,2];

pre_stim_jitter_social = pre_stim_jitter_social(randperm(length(pre_stim_jitter_social)));%randomize w/ each run
...
soPlace = 1;% index of social cue time
...
%create log files
logfile = sprintf('logfiles/AffVids_fmri_sub_%d.txt',subject_code);
...
%open log file
fid = fopen(logfile,'a+');
fprintf(fid,'Date: %s Subject: %d Run: %d\n',datestr(now), subject_code, run);
...
vidlogfile = sprintf('data/AffVids_RunList_sub_%d.mat',subject_code);%make sure loading is correct
load(vidlogfile); %load runList


trial_struct = runList(run,:);
...
tstring = sprintf('Run %i of 5 \n\n\n The task is about to begin',run);
...
WaitSecs(10); %delay before movie.
...
for i = 1:ntrials %iterate through movies...
    ...
    elseif strcmp(vid_cat(1:2), 'Sp')
        ...
        vid_category = 2;
        pre_stim_jitter = pre_stim_jitter_social(soPlace);
        soPlace = soPlace +1;
...
%     
    ...
    trial_duration = 38 - pre_stim_jitter;
    if trial_duration - (GetSecs - anchor - vidstart) > 0
        %fprintf('Current time: %s \n',GetSecs - anchor);
        %fprintf('trial has: %s \n',GetSecs - anchor -vidstart);
        %fprintf('Wait time: %s \n',trial_duration - (GetSecs - anchor - vidstart));
        WaitSecs(trial_duration - (GetSecs - anchor - vidstart)); %delay after movie to make each trial 40 seconds.
    end
    
    %% log values

    % fprintf(fid,'%s %1.3f %1.3f %1.3f %d %d %d %d ',...
    fprintf(fid,'%s %d %d %d %d ', current_trial.stimulus, current_trial.vidnum, vid_category, current_trial.condition, run);
    fprintf(fid,'%1.3f %1.3f %1.3f %1.3f ',vidstart,vidend, vidstart-vidend, current_trial.movie_object.Duration);
...
fclose(fid);
DrawFormattedText(windowptr, 'This run is complete.', 'center', 'center');%Draw text
...
% Flow of events:
% Run 1: 6 novel videos, 6 familiar videos
% Run 2: 6 novel videos, 6 familiar videos
% Run 3: 12 novel videos (4 heights, 4 social, 4 spiders)
% Run 4: 12 novel videos (4 heights, 4 social, 4 spiders)
% Run 5: 12 novel videos (4 heights, 4 social, 4 spiders)